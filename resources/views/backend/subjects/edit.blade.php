        ...
        <div class="table w-full mt-8 bg-white rounded">
            <form action="{{ route('subject.update',$subject->id) }}" method="POST" class="w-full max-w-xl px-6 py-12">
                ...
                <div class="md:flex md:items-center mb-6">
                    ...
                    <div class="md:w-2/3 block text-gray-600 font-bold">
                        <div class="relative">
                            <select name="module_id" class="block appearance-none w-full bg-gray-200 border border-gray-200 text-gray-700 py-3 px-4 pr-8 rounded leading-tight focus:outline-none focus:bg-white focus:border-gray-500" id="grid-state">
                                <option value="">--Select Module--</option>
                                @foreach ($modules as $module)
                                <option value="{{ $module->id }}"
                                        {{ ($module->id === $subject->module_id) ? 'selected' : '' }}
                                    >
                                        {{ $module->name }}
                                    </option>
                                @endforeach
