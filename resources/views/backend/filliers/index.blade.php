...
@section('content')
    <div class="roles-permissions">
        <div class="flex items-center justify-between mb-6">
            <div>
                <h2 class="text-gray-700 uppercase font-bold">Fillier</h2>
            </div>
            <div class="flex flex-wrap items-center">
                <a href="{{ route('filliers.create') }}" class="bg-gray-200 text-gray-700 text-sm uppercase py-2 px-4 flex items-center rounded">
        ...
        <div class="mt-8 bg-white rounded border-b-4 border-gray-300">
            <div class="flex flex-wrap items-center uppercase text-sm font-semibold bg-gray-300 text-gray-600 rounded-tl rounded-tr">
                ...
                <div class="w-3/12 px-4 py-3">CHEF</div>
                <div class="w-3/12 px-4 py-3">Description</div>
                <div class="w-3/12 px-4 py-3 text-right">Action</div>
            </div>
            @foreach ($subjects as $subject)
                <div class="flex flex-wrap items-center text-gray-700 border-t-2 border-l-4 border-r-4 border-gray-300">
                    <div class="w-3/12 px-4 py-3 text-sm font-semibold text-gray-600 tracking-tight">{{ $subject->fillier_name }}</div>
                    
                
                    
                    <div class="w-3/12 px-4 py-3 text-sm font-semibold text-gray-600 tracking-tight">{{ $subject->teacher->user->name }}</div>
                    <div class="w-3/12 px-4 py-3 text-sm text-gray-600 tracking-tight">{{ $subject->fillier_description }}</div>
                    <div class="w-3/12 flex items-center justify-end px-3">
                        <a href="{{ route('filliers.edit',$subject->id) }}">
                        ...
                        <form action="{{ route('filliers.destroy',$subject->id) }}" method="POST" class="inline-flex ml-1">
                            @csrf
                            @method('DELETE')
