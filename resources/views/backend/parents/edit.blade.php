        ...
        <div class="table w-full mt-8 bg-white rounded">
            <form action="{{ route('parents.update',$parent->id) }}" method="POST" class="w-full max-w-xl px-6 py-12" enctype="multipart/form-data">
                ...
                <div class="md:flex md:items-center mb-6">
                    ...
                    <div class="md:w-2/3">
                        <input name="name" class="bg-gray-200 appearance-none border-2 border-gray-200 rounded w-full py-2 px-4 text-gray-700 leading-tight focus:outline-none focus:bg-white focus:border-blue-500" type="text" value="{{ $parent->name }}">
                        @error('name')
                            ...
                            <select name="teacher_id" class="block appearance-none w-full bg-gray-200 border border-gray-200 text-gray-700 py-3 px-4 pr-8 rounded leading-tight focus:outline-none focus:bg-white focus:border-gray-500" id="grid-state">
                                ...
                                @foreach ($teachers as $teacher)
                                    <option value="{{ $teacher->id }}"
                                        {{ ($teacher->id === $parent->teacher_id) ? 'selected' : '' }}
                                    >
                                        {{ $teacher->user->name }}
                                    </option>
                                @endforeach
