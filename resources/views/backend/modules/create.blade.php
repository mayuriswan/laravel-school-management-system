...
@section('content')
    <div class="roles">
        ...
        <div class="flex items-center justify-between mb-6">
            <div>
                <h2 class="text-gray-700 uppercase font-bold">Create Module</h2>
            </div>
        ...
        <div class="table w-full mt-8 bg-white rounded">
            <form action="{{ route('module.store') }}" method="POST" class="w-full max-w-xl px-6 py-12">
                @csrf
                ...
                <div class="md:flex md:items-center mb-6">
                    ...
                    <div class="md:w-2/3">
                        <input name="module_code" class="bg-gray-200 appearance-none border-2 border-gray-200 rounded w-full py-2 px-4 text-gray-700 leading-tight focus:outline-none focus:bg-white focus:border-blue-500" type="number" value="{{ old('module_code') }}">
                        @error('module_code')
                            <p class="text-red-500 text-xs italic">{{ $message }}</p>
                        @enderror
                    </div>
